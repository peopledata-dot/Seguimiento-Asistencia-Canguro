...
// Cambiamos Firestore por Database
...
export default function App() {
  ...
  // --- FETCH DATOS DESDE REALTIME DATABASE ---
  const fetchDatos = useCallback(() => {
    ...
    return onValue(dbRef, (snapshot) => {
      ...
      if (data) {
        // Convertimos el objeto o array de Firebase a lista para React
  ...
  const handleGuardar = async (id, statusActual) => {
    if (window.confirm("¿Confirmar en Matriz?")) {
      try {
        // Buscamos la ruta correcta en Realtime
        const updates = {};
        updates[`/personal/${id}/status`] = statusActual;
        updates[`/personal/${id}/bloqueado`] = true;
        
        await update(ref(db), updates);
        // El estado personal se actualiza solo gracias a onValue
